      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span className={`text-xs font-medium ${
            outOfStock ? 'text-danger' : isLowStock ? 'text-warning' : 'text-white/40'
          }`}>
...
export function NuevaVenta({ onToast }: Props) {
  ...
  if (showSuccess) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 pt-20 animate-scale-in">
      ...
      <div className="flex-1 overflow-y-auto scrollable px-4 space-y-3">
        ...
        {cart.length === 0 && !query && (
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-white/30 text-sm">Busca un producto para agregar al carrito</p>
          </div>
        )}
      </div>
