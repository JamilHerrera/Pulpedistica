...
import { Home, TrendingUp, ShoppingCart, Package, BarChart2, LogOut } from 'lucide-react'
          ...
          {NAV_ITEMS.map(({ id, icon: Icon, label }) => {
            ...
            return (
              <button
                ...
                onClick={() => onChange(id)}
                aria-current={isActive ? 'page' : undefined}
                className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl no-select transition-all active:scale-90"
              >
                <Icon
                  size={19}
