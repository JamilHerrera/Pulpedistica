...
/** Lo que devolvera el proximo insert. Se define en cada prueba. */
const proximoInsert: { valor: Promise<unknown> } = { valor: Promise.resolve({ data: null, error: null }) }
...
vi.mock('./supabase', () => ({
  supabase: {
    from: () => ({
      insert: (fila: unknown) => {
        insertMock(fila)
        return { select: () => ({ single: () => proximoInsert.valor }) }
      },
...
describe('insertarIdempotente', () => {
  it('adjunta la clave a la fila que inserta', async () => {
    proximoInsert.valor = Promise.resolve({ data: { id: 'v1' }, error: null })
  ...
  it('ante una clave repetida no duplica: recupera la fila original', async () => {
    ...
    proximoInsert.valor = Promise.resolve({ data: null, error: { code: '23505' } })
  ...
  it('propaga cualquier otro error en vez de tragárselo', async () => {
    ...
    proximoInsert.valor = Promise.resolve({ data: null, error: { code: '42501', message: 'RLS' } })
